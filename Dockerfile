...
RUN apk add --update --virtual git patch && apk add --no-cache nginx php5-fpm php5 php5-ctype php5-xml php5-gd php5-zlib php5-bz2 php5-zip php5-openssl \
    ...
    cd /www/polr/ && patch -p0 < /migration_sqlite_patch && ln -s /usr/bin/php5 /usr/bin/php && curl -sS https://getcomposer.org/installer | php && php composer.phar install --no-dev -o && \
